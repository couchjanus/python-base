...
class Database:
    ...
    def get_by_id(self, idtofind):
        ...
        self.__cursor.execute(f"SELECT name, priority, expiredate, details, status, project FROM tasks WHERE id='{idtofind}'")
    ...
    def add_project(self, name, description, deadline):
        try:
            self.__cursor.execute(f"INSERT INTO projects (name, description, deadline) VALUES('{name}', '{description}', '{deadline}')")
            self.__conn.commit()
